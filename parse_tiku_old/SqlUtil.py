...
class Mysql(object):
    ...
    #连接池对象
    __pool = None
    def __init__(self,isAutoCommit=False):        
        self.__conn= self.__getConn(setsession=['SET AUTOCOMMIT = 1'] if isAutoCommit else None)
        self.__cur=self.__conn.cursor()
        self.__cur_dic = self.__conn.cursor(MySQLdb.cursors.DictCursor)        
    @staticmethod
    def __getConn(setsession=None):
        """ 静态方法，从连接池中取出连接 """  
        if Mysql.__pool is None:
            __pool=PooledDB(creator=MySQLdb,mincached=db['mincached'],maxcached=db['maxcached'],maxconnections=db['maxconnections'],
            host=db['host'], port=db['port'], user=db['user'], passwd=db['passwd'],  
            db=db['db'],use_unicode=False,charset=db['charset'],setsession=setsession)
